		...
		test.schema =
			...
			2: {
				...
				data: [ 1,
					...
					getImagesPromise: (next, ret, specs) ->
				...
				data: [ 1,
					...
					constructor: ->
						@events.on 'getUserImageNames', =>
							@getUserImageNames.apply @, arguments
				]
			}
			# image resize server
			4: {
...
#								spec.name, spec.size.width, spec.size.height
		...
		flow.exec(
			...
			-> # start
				...
				Object.keys(test.schema).forEach (server) =>
					...
					scope.constructor && scope.constructor.call scope
	...
	it 'should provide user images with a correct size using events', (next) ->
		...
		flow.exec(
			...
			->
				...
				w2u.events.emit('getUserImageNames', @)
			...
			(images) ->
				...
				w2i.events.emit('getImagesPromise', specs, @)
			...
			(promises) ->
				...
				promises.forEach (promise) =>
					spec = promise.specs
					w2r.events.emit('resizeImage', spec, @MULTI())
