...
#		name = @setName name
#		console.log context
##		context = context and context.__super__
#		console.log  context
##		allow to just bind to a existing signal by declaration
##		TODO move to generated init
##		if context?[ name ]?.inited isnt undefined
##			console.log 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
##			init = context[ name ].init
##			context[ name ].init = (set, next) ->
##				p = do context[ name ]
##				p.on data.on if data?.on
##				p.after data?.after if data?.after
##				p.before data?.before if data?.before
##				init.apply set, next
##
...
class SignalsMixin
	...
	getSignals: (klass = null) ->
		for prop of klass?.prototype or @
			...
			# TODO refactor inited
			continue if @[prop]?.set is undefined
			prop
