...
#			expect(scope.baz_init).was.called()
	...
	beforeEach ->
		...
		class Klass
			foo: signal('foo', on:  scope.foo_on[1] )

			on: ->
			emit: ->

		class Klass2 extends Klass
			foo: signal('foo', on:  scope.foo_on[2] )

		class Klass3 extends Klass
			foo: signal('foo', on:  scope.foo_on[3] )
		...
		it 'should merge signals defined in subclasses', (next) ->
			obj.foo()
			expect(listener).to.be.ok() for listener in scope.foo_on
